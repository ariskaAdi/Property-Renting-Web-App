...
export const sendUserBookingConfirmation = async (
  ...
  if (!user || !propertyName || booking.booking_rooms.length === 0) {
    throw new AppError(
      "Cannot send confirmation: booking data is incomplete.",
      500
    );
  }

  const formattedRooms: FormattedRoom[] = booking.booking_rooms.map(
    (dbRoom) => {
...
export const getRoomAmount = async (
  roomId: string,
  checkIn: Date,
  checkOut: Date
) => {
...
export const proofUploadService = async (
  ...
  const final_img = await updateProofImageRepository(bookingId, {
